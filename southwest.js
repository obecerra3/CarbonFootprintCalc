...
function pull_flight_info() {
    ...
    for (i = 0; i < flights.length; i++) {
        ...
        dateString += " " + times[i];
        flightProfiles.push(new FlightProfile(flights[i][2], createDate(dateString), "NoData", flights[i][0], flights[i][1]));
    }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.task === 'southwest') {
    sendResponse({ flightProfiles: flightProfiles });
  }
});